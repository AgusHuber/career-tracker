  ...
  'No Cursada': {
    ...
    icon: () => (
      <svg viewBox="0 0 24 24" role="img" aria-hidden="true">
        <circle cx="12" cy="12" r="8" stroke="currentColor" strokeWidth="2" strokeDasharray="4 4" fill="none" />
      </svg>
    ),
  },
  Cursable: {
    ...
    icon: () => (
      <svg viewBox="0 0 24 24" role="img" aria-hidden="true">
        <circle cx="12" cy="12" r="8" stroke="currentColor" strokeWidth="2" strokeOpacity="0.35" fill="none" />
        <path
          d="M9 8l6 4-6 4"
          stroke="currentColor"
          strokeWidth="2"
...
const StatusSelector = ({ value, onChange }) => (
  <div className="status-selector" role="group" aria-label="Actualizar situación">
    {STATUS_OPTIONS.map((option) => {
      ...
      return (
        <button
          type="button"
          key={option}
          className={`status-selector__option status-selector__option--${toStatusSlug(option)} ${
            isActive ? 'is-active' : ''
          }`}
...
const TUTORIAL_STEPS = [
  {
    ...
    message: 'Esta es una materia de tu plan de estudios. Aquí puedes ver su nombre, código, año y si es anual o cuatrimestral.',
    position: 'right',
  ...
  },
  {
    ...
    message: 'Esta materia no tiene correlativas, por lo que puedes cursarla desde el inicio. Otras materias mostrarán aquí sus requisitos previos.',
    position: 'right',
    highlightType: 'element'
  },
  {
    ...
    message: 'Presiona los botones para cambiar el estado y mira cómo cambia el color de la materia en tiempo real.',
    position: 'top',
  ...
  },
  {
    ...
    message: 'Tu progreso se guarda automáticamente en tu navegador. Presiona "Finalizar" para comenzar.',
    position: 'center',
    highlightType: null
  }
];
...
export function Materias({ onBack }) {
  ...
  const [showTutorial, setShowTutorial] = useState(() => {
    const tutorialCompleted = localStorage.getItem('career-tracker-tutorial-completed');
  ...
  useEffect(() => {
    localStorage.setItem('career-tracker-progress', JSON.stringify(userProgress));
  }, [userProgress]);
  ...
  const stats = useMemo(() => {
    ...
    const porCursar = materiasPlan.filter(
      (m) => computedStatuses[m.numero] === 'No Cursada' || computedStatuses[m.numero] === 'Cursable'
    ).length;

    // Suma puntos de electivas y extras aprobadas
    const puntosElectivas = [...materiasElectivas, ...materiasExtras].reduce((sum, m) => {
    ...
    return { porCursar, aprobadas, puntosElectivas, finalesPendientes, porcentajeCarrera };
  ...
  const renderMateriasList = (lista, isElectiva = false) => (
    <div className="materias-board">
      ...
      <div className="materias-list">
        {lista.map((materia) => {
          const estadoCalculado = computedStatuses[materia.numero] ?? 'No Cursada';
          ...
          const highlightAlsoRowOnStatusStep =
            ...
            materia.numero === 1;
          const isHighlighted = highlightElement || highlightAlsoRowOnStatusStep;

          return (
            <article
              key={materia.numero}
              id={`materia-${materia.numero}`}
              className={`materia-row materia-row--${rowStateClass} ${isHighlighted ? 'tutorial-highlight' : ''}`}
            >
              ...
              <div className="materia-row__cell materia-row__cell--title">
                ...
                <span className="materia-row__subcode">
                  <span className="materia-row__anio">{getAnioLabel(materia.anio)} - </span>
                  {materia.codigo}
                  <span className="materia-row__cuatrimestre"> • {getCuatrimestreLabel(materia.cuatrimestre)}</span>
                  {(isElectiva || materia.extra) && materia.puntos && (
                    <span className="materia-row__puntos"> • {materia.puntos} pts</span>
                  )}
                </span>
              </div>
              ...
              >
                {materia.prerequisitos?.length ? (
                  <ul className={showTutorial && currentStep.target === `prereqs-${materia.numero}` ? 'tutorial-highlight' : ''}>
                    {materia.prerequisitos.map((pr) => {
                      const prereqStatus = computedStatuses[pr.numero] ?? 'No Cursada';
                      ...
                      return (
                        <li key={`${materia.numero}-${pr.numero}-${pr.tipo}`}>
                          <button
                            type="button"
                            className={`prereq-chip prereq-chip--${isSatisfied ? toStatusSlug(prereqStatus) : 'no-cursada'}`}
                            data-tooltip={prereqMateria?.nombre ?? 'Materia'}
                          >
                            <span className="prereq-chip__code">{pr.numero}</span>
                  ...
                  <span 
                    className={`materia-row__empty ${showTutorial && currentStep.target === `prereqs-${materia.numero}` ? 'tutorial-highlight' : ''}`}
                    id={`prereqs-${materia.numero}`}
                  >
                    —
                  </span>
              ...
              >
                {estadoCalculado === 'No Cursada' && !['Aprobado', 'Regular'].includes(estadoManual) ? (
                  <div
                    className={`status-blocked ${
                      showTutorial && currentStep.target === `status-selector-${materia.numero}` && currentStep.highlightType === 'statuses'
                        ? 'tutorial-highlight'
                        : ''
                    }`}
                  >
                  ...
                  >
                    <StatusSelector
                      value={estadoManual}
                      onChange={(estado) => handleStatusChange(materia.numero, estado)}
                    />
                  </div>
                )}
              </div>
    ...
    <div className="materias">
      <div className="materias-shell">
        ...
        <div className="materias-tabs">
          <button
            className={`materias-tab ${activeTab === 'plan' ? 'is-active' : ''}`}
          ...
          {materiasElectivas.length > 0 && (
            <button
              className={`materias-tab ${activeTab === 'electivas' ? 'is-active' : ''}`}
          ...
          {materiasExtras.length > 0 && (
            <button
              className={`materias-tab ${activeTab === 'extras' ? 'is-active' : ''}`}
        ...
        {activeTab === 'electivas' && renderMateriasList(materiasElectivas, true)}
      ...
      <div 
        id="stats-panel"
        className={`stats-panel ${panelExpanded ? 'stats-panel--expanded' : 'stats-panel--collapsed'} ${
          showTutorial && TUTORIAL_STEPS[tutorialStep].target === 'stats-panel' ? 'tutorial-highlight' : ''
        }`}
      >
          ...
          <div className="stats-panel__content">
            ...
            <div className="stats-panel__group">
              <div className="stats-panel__item stats-panel__item--highlight">
                ...
                <span className="stats-panel__value stats-panel__value--large">{stats.porcentajeCarrera}%</span>
              </div>

              <div className="stats-panel__item">
                <span className="stats-panel__label">Puntos Electivas</span>
                <span className="stats-panel__value">{stats.puntosElectivas}/20</span>
              </div>
            </div>
            ...
            <div className="stats-panel__group">
              ...
              <div className="stats-panel__item stats-panel__item--finales">
                ...
                <span className="stats-panel__value">{stats.finalesPendientes}</span>
              </div>
            </div>
          </div>
        )}
      ...
      {showTutorial && (
        <>
          ...
          <div className={`tutorial-dialog tutorial-dialog--${TUTORIAL_STEPS[tutorialStep].position}`}>
            ...
            <div className="tutorial-dialog__content">
              <p style={{ whiteSpace: 'pre-line' }}>{TUTORIAL_STEPS[tutorialStep].message}</p>
            </div>
